...
module.exports = function generatePDF(options, templates, newLogger, appendFiles) {
	logger = newLogger
	...
	return Promise.all(templates.map((html, index) => convertToPdf(html, index, options)))
		.then((files) => {
			...
			return merge(files, appendFiles, options.fileName, options.pdftkPath)
		})
...
//merge pdfs
function merge(files, appendFiles = [], fileName) {
	//logger.info("Merging files")
	let pdftkPath = "pdftk"
	let pdfFiles = files.join(" ")
	let htmlFiles = replaceall(".pdf", "html.html", pdfFiles)
	let tempMergedPath = tempDir + fileName
	let cmd = [pdftkPath, pdfFiles, appendFiles.join(" "), "cat output", tempMergedPath].join(" ")
	let startTime = Date.now()
	return pexec(cmd)
		.then(() => {
