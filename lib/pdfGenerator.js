...
function merge(files, fileName, pdftkPath) {
	...
	let pdftkPath = pdftkPath || "pdftk"
	let pdfFiles = files.join(" ")
	let htmlFiles = pdfFiles.replaceAll(".pdf", "html.html")
	...
	return pexec(params)
		.then(() => {
			logger.info("Merge done, now removing temporary pdf files")
			return pexec("rm " + pdfFiles) //del
		})
		.then(() => {
			logger.info("Merge done, now removing temporary html files")
			return pexec("rm " + htmlFiles) //del
		})
		.then(() => {
